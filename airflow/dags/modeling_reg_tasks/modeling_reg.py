...
def load_env_variables(env_file='/opt/airflow/config/.env'):
...
def load_data_to_mysql(merged_df, user, password, host, port, database, table_name):
    ...
    # Open a connection and execute raw SQL for ignoring duplicates
    with engine.connect() as conn:
        for index, row in merged_df.iterrows():
            insert_stmt = text(f"""
                INSERT IGNORE INTO {table_name} (sku, estimated_price, number_of_products_sold, stock_levels, lead_times, order_quantities, shipping_times, shipping_carriers, shipping_costs, location, production_volumes, manufacturing_lead_time, manufacturing_costs, inspection_results, defect_rates, transportation_modes, routes, costs, demurrage_cost_prediction, demurrage_bin)
                VALUES (:sku, :estimated_price, :number_of_products_sold, :stock_levels, :lead_times, :order_quantities, :shipping_times, :shipping_carriers, :shipping_costs, :location, :production_volumes, :manufacturing_lead_time, :manufacturing_costs, :inspection_results, :defect_rates, :transportation_modes, :routes, :costs, :demurrage_cost_prediction, :demurrage_bin)
            """)
            conn.execute(insert_stmt, **row.to_dict())

    print("Data loaded successfully, ignoring duplicates.")


def main_modeling_reg():
    load_env_variables()
